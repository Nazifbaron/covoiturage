...
<div class="max-w-2xl mx-auto h-[calc(100vh-7rem)] flex flex-col">
    ...
    <div class="flex items-center gap-3 px-4 py-3
        ...
        {{-- ← Retour vers les demandes conducteur --}}
        ...
        {{-- Avatar du passager (interlocuteur du conducteur) --}}
        ...
        <div class="flex-1 min-w-0">
            ...
            <p class="text-xs font-semibold text-slate-400">
                <span class="material-symbols-outlined" style="font-size:12px;vertical-align:middle">hail</span>
                Passager
            </p>
        </div>
...
// Routes Laravel générées côté serveur → jamais d'URL en dur
...
// Initiales de l'interlocuteur (passager) pour les avatars JS
...
async function poll() {
    try {
        ...
        // Clignote pour signaler la synchro
...
function appendMessage(msg) {
    // parseInt pour éviter "5" === 5 qui échoue en JS strict
    ...
    const avatarOther = `<div class="w-7 h-7 rounded-lg bg-gradient-to-br from-violet-400 to-purple-600
...
async function markRead() {
    try {
        await fetch(URL_READ, { method: 'POST', headers: { 'X-CSRF-TOKEN': CSRF_TOKEN, 'X-Requested-With': 'XMLHttpRequest' } });
    } catch(e) {}
}
