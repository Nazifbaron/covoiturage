...
<div id="bookModal"
    ...
    <div id="bookModalBox"
         class="relative w-full max-w-md bg-white dark:bg-card-dark rounded-2xl shadow-2xl
                border border-slate-100 dark:border-white/10 p-6 space-y-5
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-xs text-slate-400 mt-0.5" id="modal-driver">—</p>
            </div>
        ...
        <div id="modal-vehicle-block"
             class="flex items-center gap-3 p-3 rounded-xl bg-primary/5 border border-primary/15">
            <span class="material-symbols-outlined text-primary text-2xl" id="modal-vehicle-icon">directions_car</span>
            <div>
                <p class="text-xs font-black text-slate-900 dark:text-white" id="modal-vehicle-name">—</p>
                <p class="text-xs text-slate-400 font-semibold" id="modal-vehicle-plate">—</p>
            </div>
        </div>
        ...
        <form id="book-form" method="POST" action="{{ route('passenger.storetrips') }}" class="space-y-4">
            ...
            {{-- Message optionnel --}}
            <div>
                <label class="text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 block">Message au conducteur (optionnel)</label>
                <textarea name="message" rows="2" placeholder="Ex : Je serai devant la pharmacie..."
...
// ══════════════════════════════════════════════
//  INIT : charger tous les trajets au démarrage
// ══════════════════════════════════════════════
...
async function fetchTrips(page = 1) {
    ...
    try {
        ...
        // Si réponse non-JSON (ex: erreur 500 HTML), attraper proprement
        ...
        try {
            ...
            document.getElementById('results-count').textContent = 'Erreur serveur — voir console.';
        ...
        if (!data.success && data.error) {
            console.error('Erreur contrôleur :', data.error);
        }
...
function renderTrips(trips, pagination, total) {
    ...
    trips.forEach(trip => {
        ...
        // Véhicule
        const vehicleHtml = trip.vehicle ? `
            <div class="flex items-center gap-2 mt-3 pt-3 border-t border-slate-100 dark:border-white/5">
                ...
                <span class="text-xs font-semibold text-slate-500 dark:text-slate-400">${trip.vehicle.brand} ${trip.vehicle.model}</span>
                <span class="text-xs font-black text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/10
                             px-2 py-0.5 rounded-lg tracking-wider ml-auto">${trip.vehicle.plate.toUpperCase()}</span>
            </div>` : '';
...
function renderPagination(p) {
    ...
    const btn = (label, page, disabled, active) => {
        if (disabled) return `<span class="w-9 h-9 rounded-xl bg-white dark:bg-card-dark
            border border-slate-100 dark:border-primary/10 flex items-center justify-center opacity-30 cursor-not-allowed text-sm">${label}</span>`;
    ...
    for (let i = start; i <= end; i++) {
        pagDiv.innerHTML += btn(i, i, false, i === p.current_page);
    }
...
function openBookModal(trip) {
    ...
    document.getElementById('modal-driver').textContent    = `Conducteur : ${trip.driver_name}`;
    ...
    if (trip.vehicle) {
        ...
        document.getElementById('modal-vehicle-block').classList.remove('hidden');
    } else {
        document.getElementById('modal-vehicle-block').classList.add('hidden');
    }
...
function changeSeat(delta) {
    ...
    val = Math.max(1, Math.min(modalMaxSeats, val));
    el.textContent = val;
    inp.value      = val;
    updateTotal();
}
